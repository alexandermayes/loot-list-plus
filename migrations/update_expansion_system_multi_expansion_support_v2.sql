-- =====================================================
-- UPDATE EXPANSION SYSTEM: Multi-Expansion Support (v2)
-- =====================================================
...
-- =====================================================
-- STEP 3: Fix loot_submissions table status column
-- =====================================================

-- First, check what status values currently exist and fix them
DO $$
BEGIN
  ...
  IF NOT EXISTS (
    ...
    RAISE NOTICE 'Added status column to loot_submissions';
  END IF;

  -- Update any NULL status values to 'approved' (legacy data)
  UPDATE loot_submissions
  SET status = 'approved'
  WHERE status IS NULL;

  -- Update any invalid status values to 'approved'
  UPDATE loot_submissions
  SET status = 'approved'
  WHERE status NOT IN ('pending', 'approved', 'rejected');

  RAISE NOTICE 'Updated loot_submissions status values';
END $$;
