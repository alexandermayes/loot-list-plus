...
export default function LoginPage({ nextParam = null, isAuthenticated = false, authError = null }: LoginPageProps) {
  ...
  const handleJoinGuild = async () => {
    ...
    try {
      const response = await fetch(`/api/guild-invites/${inviteCode}`, {
        method: 'POST',
      ...
      if (!response.ok) {
        setJoinError(data.error || 'Couldn\'t join guild. Try again.')
        setJoining(false)
        return
      }
      ...
      setJoinError('Couldn\'t join guild. Check your connection.')
      setJoining(false)
    }
  }
          ...
          <div className="w-full max-w-[373px] flex flex-col items-center gap-[60px]">
            <div className="flex flex-col items-center gap-5 w-full">
              ...
              <p className="text-muted-foreground text-base text-center">
                Connect with Discord to create your guild or join one you have already been invited to. Core features are free.
              </p>
            </div>
            ...
            <div className="flex flex-col gap-2.5 w-full">
              ...
              <a
                href="https://www.getlootlist.com/#how-it-works"
              ...
              <p className="text-muted-foreground text-sm text-center mt-2">
                ...
                <Link href="/terms" className="text-foreground underline hover:text-foreground-secondary">Terms of Service</Link>{' '}
                and{' '}
                <Link href="/privacy" className="text-foreground underline hover:text-foreground-secondary">Privacy Policy</Link>.
              </p>
            </div>
          </div>
        </div>
      ...
      {guildInfo && (
        <Modal open={showInviteModal} onClose={() => setShowInviteModal(false)} size="sm">
          <ModalHeader onClose={() => setShowInviteModal(false)}>
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-lg bg-accent/10 flex items-center justify-center">
                <HugeiconsIcon icon={joinSuccess ? Tick01Icon : Key01Icon} size={20} className={joinSuccess ? 'text-success' : 'text-accent'} />
              </div>
              <div>
                <ModalTitle>{joinSuccess ? 'You\'re in' : 'You\'ve been invited'}</ModalTitle>
                <ModalDescription>
                  {joinSuccess ? 'Redirecting you now...' : user ? `Join ${guildInfo.guild.name}` : 'Sign in with Discord to join this guild'}
                </ModalDescription>
              </div>
            </div>
          </ModalHeader>
          ...
          {!joinSuccess && (
            <ModalFooter>
              {user ? (
                <Button variant="primary" onClick={handleJoinGuild} className="w-full" loading={joining}>
                  Join guild
                </Button>
              ) : (
                <Button variant="primary" onClick={handleDiscordLogin} className="w-full">
                  {discordIcon}
