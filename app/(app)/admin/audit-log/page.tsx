...
export default function AdminAuditLog() {
  ...
  const fetchHistory = useCallback(async (offset = 0, append = false) => {
    ...
    try {
      ...
      if (!response.ok) {
        showNotification('error', result.error || 'Failed to load audit log')
        return
      }
      ...
      showNotification('error', 'Failed to load audit log')
    } finally {
      setLoading(false)
  ...
  // Set page title
  useEffect(() => {
    document.title = 'LootList+ • Audit Log'
  }, [])
  ...
  return (
    <ExpansionGuard>
      <div className="p-4 sm:p-6 lg:p-8 space-y-6 font-poppins">
        {/* Header */}
        <div>
          <Heading level={1}>Loot Audit Log</Heading>
          <Text color="muted" className="mt-1">
            View all loot awards made by officers
          </Text>
        </div>
